...
const MPharm = () => {
  ...
  const fetchNotes = async () => {
    try {
      const res = await axios.get(`${BASE_URL}/api/admin/public/notes?course=M.Pharm`);
      setNotes(res.data);
  ...
  const fetchPremiumVideos = async () => {
    try {
      const res = await axios.get(`${BASE_URL}/api/admin/public/videos?course=M.Pharm`);
  ...
  const fetchFreeVideos = async () => {
    try {
      const res = await axios.get(`${BASE_URL}/api/admin/public/free-videos?course=M.Pharm`);
      const freeOnly = res.data.filter(video => video.isPremium === false);
      setFreeVideos(freeOnly);
    } catch (error) { 
      console.log(error);
      try {
        const allRes = await axios.get(`${BASE_URL}/api/admin/public/videos?course=M.Pharm`);
  ...
  const fetchPaidPDFs = async () => {
    try {
      const res = await axios.get(`${BASE_URL}/api/admin/public/paid-pdfs?course=M.Pharm`);
  ...
  const fetchPapers = async () => {
    try {
      const res = await axios.get(`${BASE_URL}/api/admin/public/papers?course=M.Pharm`);
  ...
  // ========== SEMESTER CARDS SECTION - 4 CARDS ==========
  const renderSemesterSection = (title, data, type, renderCard, icon, semesters = mpharmSemesters, noDataMessage = "No content available yet.") => {
    ...
    return (
      <div className="mb-12">
        ...
        {/* Semester Cards Grid - 2 on mobile, 4 on desktop */}
          ...
          return (
            <div key={`content-${sem}`} className="mt-6 animate-fadeIn">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h4 className="text-lg sm:text-xl font-bold text-gray-800">Semester {sem} {type === "premium-video" ? "Videos" : type === "premium-paper" ? "Papers" : "PDFs"}</h4>
    ...
    <div className="min-h-screen bg-gradient-to-br from-white via-sky-50 to-white">
        ...
        <div className="relative h-[300px] sm:h-[360px] md:h-[520px] w-full">
          ...
          <div className="relative z-20 flex items-center h-full px-4 sm:px-6 md:px-20">
            <div className="max-w-[520px]">
              <h1 className="text-white text-3xl sm:text-4xl md:text-7xl font-extrabold leading-tight mb-3 sm:mb-5">Master<br /><span className="text-sky-400">of Pharmacy</span></h1>
              <p className="text-gray-300 text-xs sm:text-sm md:text-lg leading-relaxed mb-5 sm:mb-8 max-w-[500px]">Complete Notes, Semester-wise PDFs, Practical Videos & Predictive Papers for M.Pharm Students.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-8 sm:py-14">
        ...
        <div id="semester" className="mb-16 sm:mb-24 scroll-mt-20">
          ...
          {renderSemesterSection("Semester-wise PDFs", pdfsBySemester, "paid-pdf", (item, type, idx) => renderPremiumCard(item, "paid-pdf", Lock, idx), <GraduationCap size={20} className="sm:w-6 sm:h-6 text-purple-600" />, mpharmSemesters, "No premium PDFs available yet. Upload from Admin Panel.")}
        </div>
        ...
        <div id="videos" className="mb-16 sm:mb-24 scroll-mt-20">
          ...
          {renderSemesterSection("Semester-wise Videos", videosBySemester, "premium-video", (item, type, idx) => renderPremiumVideoCard(item, idx), <Video size={20} className="sm:w-6 sm:h-6 text-purple-600" />, mpharmSemesters, "No premium videos available yet.")}
        </div>
        ...
        <div id="papers" className="mb-16 sm:mb-24 scroll-mt-20">
          ...
          {renderSemesterSection("Semester-wise Papers", papersBySemester, "premium-paper", (item, type, idx) => renderPremiumCard(item, "premium-paper", Brain, idx), <Brain size={20} className="sm:w-6 sm:h-6 text-purple-600" />, mpharmSemesters, "No premium papers available yet.")}
        </div>
      </div>
