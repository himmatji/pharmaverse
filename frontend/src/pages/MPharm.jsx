...
const MPharm = () => {
  ...
  const fetchNotes = async () => {
    try {
      const res = await axios.get("http://localhost:5000/api/admin/public/notes?course=M.Pharm");
      setNotes(res.data);
  ...
  const fetchPremiumVideos = async () => {
    try {
      const res = await axios.get("http://localhost:5000/api/admin/public/videos?course=M.Pharm");
  ...
  const fetchFreeVideos = async () => {
    try {
      const res = await axios.get("http://localhost:5000/api/admin/public/free-videos?course=M.Pharm");
      const freeOnly = res.data.filter(video => video.isPremium === false);
      setFreeVideos(freeOnly);
    } catch (error) { 
      console.log(error);
      try {
        const allRes = await axios.get("http://localhost:5000/api/admin/public/videos?course=M.Pharm");
  ...
  const fetchPaidPDFs = async () => {
    try {
      const res = await axios.get("http://localhost:5000/api/admin/public/paid-pdfs?course=M.Pharm");
  ...
  const fetchPapers = async () => {
    try {
      const res = await axios.get("http://localhost:5000/api/admin/public/papers?course=M.Pharm");
      ...
      <div key={cardId} className="group relative" style={{ animation: `cinematicReveal 0.6s cubic-bezier(0.23, 1, 0.32, 1) ${index * 0.08}s both` }}
        ...
        <div className="relative bg-white rounded-2xl overflow-hidden shadow-lg group-hover:shadow-2xl transition-all duration-500 group-hover:-translate-y-2">
          ...
          <div className="p-5 pt-7">
            ...
            <p className="text-gray-500 text-sm leading-relaxed mb-5 line-clamp-2">{item.description || "Comprehensive study material designed for M.Pharm students."}</p>
      ...
      <div key={cardId} className="group relative" style={{ animation: `cinematicReveal 0.6s cubic-bezier(0.23, 1, 0.32, 1) ${index * 0.08}s both` }}
        ...
        <div className="relative bg-white rounded-2xl overflow-hidden shadow-lg group-hover:shadow-2xl transition-all duration-500 group-hover:-translate-y-2">
          ...
          <div className="p-5 pt-7">
            ...
            <p className="text-gray-500 text-sm leading-relaxed mb-5 line-clamp-2">{item.description || "Premium study material for M.Pharm excellence."}</p>
            <div className="flex gap-2.5">
              {isLocked ? (
  ...
  // ========== SEMESTER CARDS SECTION - B.PHARM JESA ==========
  const renderSemesterSection = (title, data, type, renderCard, icon, semesters = mpharmSemesters, noDataMessage = "No content available yet.") => {
    ...
    return (
      <div className="mb-12">
        ...
        {/* Semester Cards Grid - 4 cards in a row */}
          ...
          return (
            <div key={`content-${sem}`} className="mt-6 animate-fadeIn">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h4 className="text-xl font-bold text-gray-800">Semester {sem} {type === "premium-video" ? "Videos" : type === "premium-paper" ? "Papers" : "PDFs"}</h4>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-white via-sky-50 to-white">
        ...
        <div className="relative h-[360px] md:h-[520px] w-full">
          ...
          <div className="relative z-20 flex items-center h-full px-6 md:px-20">
            <div className="max-w-[520px]">
              <h1 className="text-white text-4xl md:text-7xl font-extrabold leading-tight mb-5">Master<br /><span className="text-sky-400">of Pharmacy</span></h1>
              <p className="text-gray-300 text-sm md:text-lg leading-relaxed mb-8 max-w-[500px]">Complete Notes, Semester-wise PDFs, Practical Videos & Predictive Papers for M.Pharm Students.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
        ...
        {/* PREMIUM PDFS - SEMESTER CARDS (4 CARDS) */}
        <div id="semester" className="mb-24 scroll-mt-20">
          ...
          {renderSemesterSection("Semester-wise PDFs", pdfsBySemester, "paid-pdf", (item, type, idx) => renderPremiumCard(item, "paid-pdf", Lock, idx), <GraduationCap size={24} className="text-purple-600" />, mpharmSemesters, "No premium PDFs available yet. Upload from Admin Panel.")}
        </div>

        {/* PREMIUM VIDEOS - SEMESTER CARDS */}
        <div id="videos" className="mb-24 scroll-mt-20">
          <div className="text-center mb-14">
            ...
            <div className="w-24 h-1 bg-gradient-to-r from-sky-400 to-blue-500 mx-auto rounded-full mt-4"></div>
            <p className="text-gray-500 mt-2">Click on any semester card to view its videos</p>
          </div>
          
          {renderSemesterSection("Semester-wise Videos", videosBySemester, "premium-video", (item, type, idx) => renderPremiumVideoCard(item, idx), <Video size={24} className="text-purple-600" />, mpharmSemesters, "No premium videos available yet.")}
        </div>

        {/* PREMIUM PAPERS - SEMESTER CARDS */}
        <div id="papers" className="mb-24 scroll-mt-20">
          <div className="text-center mb-14">
            ...
            <div className="w-24 h-1 bg-gradient-to-r from-sky-400 to-blue-500 mx-auto rounded-full mt-4"></div>
            <p className="text-gray-500 mt-2">Click on any semester card to view its papers</p>
          </div>
          
          {renderSemesterSection("Semester-wise Papers", papersBySemester, "premium-paper", (item, type, idx) => renderPremiumCard(item, "premium-paper", Brain, idx), <Brain size={24} className="text-purple-600" />, mpharmSemesters, "No premium papers available yet.")}
        </div>
      </div>
    </div>
  );
