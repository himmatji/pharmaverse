...
import bannerImg from "../assets/pharmaD.webp";
...
const PharmD = () => {
  ...
  const fetchPremiumVideos = async () => {
    try {
      const res = await axios.get(`${API_URL}/public/videos?course=Pharm.D`);
  ...
  const fetchFreeVideos = async () => {
    try {
      const res = await axios.get(`${API_URL}/public/free-videos?course=Pharm.D`);
      const freeOnly = res.data.filter(video => video.isPremium === false);
      setFreeVideos(freeOnly);
    } catch (error) { 
      console.log(error);
      try {
        const allRes = await axios.get(`${API_URL}/public/videos?course=Pharm.D`);
  ...
  const fetchPaidPDFs = async () => {
    try {
      const res = await axios.get(`${API_URL}/public/paid-pdfs?course=Pharm.D`);
  ...
  const fetchPapers = async () => {
    try {
      const res = await axios.get(`${API_URL}/public/papers?course=Pharm.D`);
          ...
          <div className="p-5 pt-7">
            ...
            {type === "free-paper" && (
              <div className="flex items-center gap-1.5 mb-3 flex-wrap">
                {item.semester && <span className="text-xs font-medium bg-emerald-50 text-emerald-700 px-2.5 py-1 rounded-lg">Semester {item.semester}</span>}
                {item.year && <span className="text-xs font-medium bg-emerald-50 text-emerald-700 px-2.5 py-1 rounded-lg">Year {item.year}</span>}
                {item.difficulty && <span className={`text-xs font-medium px-2.5 py-1 rounded-lg ${item.difficulty === "Hard" ? "bg-red-50 text-red-600" : item.difficulty === "Expert" ? "bg-purple-50 text-purple-600" : "bg-green-50 text-green-600"}`}>{item.difficulty}</span>}
              </div>
            )}
            <p className="text-gray-500 text-sm leading-relaxed mb-5 line-clamp-2">{item.description || "Comprehensive study material designed for Pharm.D students."}</p>
        ...
        <div className="relative bg-white rounded-2xl overflow-hidden shadow-lg group-hover:shadow-2xl transition-all duration-500 group-hover:-translate-y-2">
          ...
          <div className="p-5 pt-7">
            ...
            {(type === "paid-pdf" || type === "premium-paper") && (
              <div className="mb-3">
                {item.semester && <span className="text-xs font-medium bg-purple-50 text-purple-700 px-2.5 py-1 rounded-lg mr-2">Semester {item.semester}</span>}
                {item.year && <span className="text-xs font-medium bg-purple-50 text-purple-700 px-2.5 py-1 rounded-lg">Year {item.year}</span>}
              </div>
            )}
            <p className="text-gray-500 text-sm leading-relaxed mb-5 line-clamp-2">{item.description || "Premium study material for Pharm.D excellence."}</p>
            <div className="flex gap-2.5">
              {isLocked ? (
