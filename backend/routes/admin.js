...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      let parsedUnits = [];
      try {
        parsedUnits = units ? JSON.parse(units) : [];
      } catch (e) {
        parsedUnits = [];
      }
      ...
      if (type === "note") {
        ...
        return res.status(400).json({
          success: false,
          message: "Invalid file type. Use 'note', 'video', or 'paper'"
        });
      }
      ...
      const contentData = {
        ...
        description: description || `${category} for ${subject}`,
        course: branch || "B.Pharm",
        branch: branch || "B.Pharm",
        category: category,
      ...
      console.log("✅ Document ID:", newContent._id);
      console.log("=================================\n");
...
router.put("/course-prices", adminAuth, async (req, res) => {
  try {
    ...
    const formattedPrices = {
      ...
      PhD: prices.PhD || { price: 199, discount: 0 }
    };
...
router.get("/public-price", async (req, res) => {
  try {
    ...
    const getPrice = (key) => {
      if (!p) return null;
      if (typeof p.get === "function") return p.get(key);
      return p[key];
    };

    res.json({
      "B.Pharm": getPrice("BPharm"),
      "D.Pharm": getPrice("DPharm"),
      "M.Pharm": getPrice("MPharm"),
      "Pharm.D": getPrice("PharmD"),
      "PhD": getPrice("PhD")
    });
